      ...
      if (import.meta.env.DEV) {
        ...
        items.forEach((item, idx) => {
          console.log(`📋 Alert ${idx + 1}:`, {
            ...
            intakeFields: item.intake ? Object.keys(item.intake) : [],
            user: item.user?.name || 'Unknown',
          });
        });
        const itemsWithIntake = items.filter(i => i.intake);
        console.log(`📋 Alerts with intake data: ${itemsWithIntake.length} out of ${items.length}`);
      }
    } catch (e) {
      const msg =
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <h1 className="text-lg sm:text-xl md:text-2xl font-semibold text-slate-900 dark:text-white">Therapy Requests</h1>
        <button
          onClick={() => {
            load();
            loadNotifications();
          }}
          className="px-3 py-2 rounded-2xl border border-slate-300 dark:border-white/20 bg-slate-200 dark:bg-white/10 text-slate-900 dark:text-white hover:bg-slate-300 dark:hover:bg-white/20 transition"
        >
          Refresh
        </button>
      </div>
      ...
      {/* Card container */}
      <div className="rounded-2xl sm:rounded-3xl border border-slate-300 dark:border-white/10 bg-gradient-to-b from-slate-50/90 dark:from-slate-900/70 to-slate-100/90 dark:to-slate-800/60 backdrop-blur p-3 sm:p-4 text-slate-900 dark:text-white shadow-xl">
        {err && (
        ...
        {!loading && items.length === 0 && !err && (
          <div className="p-8 text-slate-700 dark:text-slate-300 text-center">
            ...
            <div className="text-sm text-slate-600 dark:text-slate-400">When patients send therapy requests, they will appear here.</div>
          </div>
        )}
